...
export interface ItemMasonry {
  id: number;
  img: string;
  height: number;
}

interface GridItem extends ItemMasonry {
  x: number;
  y: number;
  w: number;
  h: number;
...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const handlePreviousClick = () => {
    if (viewItemId !== null) {
      const currentIndex = items.findIndex((item) => item.id === viewItemId);
      const previousIndex = (currentIndex - 1 + items.length) % items.length;
      setViewItemId(items[previousIndex].id);
    }
  };
  ...
  const handleNextClick = () => {
    if (viewItemId !== null) {
      ...
      setViewItemId(items[nextIndex].id);
    }
  };
    ...
    <div ref={containerRef} className="relative w-full h-full">
      {!imagesReady && (
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            {/* Kamu bisa ganti ini dengan Spinner/Lottie */}
      ...
      {viewItemId && (
        ...
        >
          ...
          >
            ...
            <img
              draggable={false}
              className="object-contain max-h-[90vh] rounded-lg md:rounded-none no-drag"
              src={items.find((item) => item.id === viewItemId)?.img}
              alt={items
                .find((item) => item.id === viewItemId)
                ?.img.split("/")
                .pop()}
            />
            <button className="button-gallery -ml-15 sm:-ml-18  md:-ml-20 lg:ml-0" onClick={handleNextClick}>
              <ChevronRight color="#000000" className="w-12 h-12" />
            </button>
          </motion.div>
        </motion.div>
      )}
